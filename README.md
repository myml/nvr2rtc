...
nvr2rtc --user admin --pass 123456  --ch 0,2,3,5
```

- 输出端点：`http://<host>:8081/ch/<N>`（N = API 通道号，0 起；本机器 0/2/3/5 =
  NVR 的第 1/3/4/6 路）
- 默认**透传原始流**（含 TP-Link 私有流 0x92 与 null 填充）；加 `--clean` 开关做 TS 清洗
  （只保留视频 PID，剔除私有流）——清洗不影响启动速度，只是让下游工具看到的流更干净
- 断流自动重连；一个会话只服务一个通道，多通道各自独立连接 NVR
...
| `--ch` | 不限制 | 通道白名单（逗号分隔，如 `0,2,3`）。**默认不限制**——任意非负整数通道都放行，
...
streams:
  cam0:
    ffmpeg: http://192.168.0.60:8081/ch/0#video=copy
  cam2:
    ffmpeg: http://192.168.0.60:8081/ch/2#video=copy
```
...
streams:
  cam0:
    ffmpeg2:
      ...
      - "100000"
      - -i
      - http://192.168.0.60:8081/ch/0
```

命令行直接拉流：

```bash
ffmpeg -probesize 32768 -analyzeduration 100000 -i http://<host>:8081/ch/0 -c copy out.ts
ffprobe http://<host>:8081/ch/0
```
...
[Service]
ExecStart=/nas/nvr2rtc/nvr2rtc --pass 你的密码 --ch 0,2,3,5
Restart=always
RestartSec=5
